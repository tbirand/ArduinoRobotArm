	...
	difference() {
		union() {
			...
			difference() {
				...
				*ranslate([0, 18, 5.1]) cube([30, 10, 2], center=true);
			}
		}
		translate([0,0,0.5]) cylinder(r=2.5,h=10,$fn=45);
		translate([0,0,-3]) cylinder(r=1,h=10,$fn=45);

		for(i=[4.5:1.5:16]) {
			
			translate([i,0,-0.05]) cylinder(r=0.5,h=3,$fn=45);
			translate([-i,0,-0.55]) cylinder(r=0.5,h=3,$fn=45);
			translate([0,i,-0.05]) cylinder(r=0.5,h=3,$fn=45);
			translate([0,-i,-0.05]) cylinder(r=0.5,h=3,$fn=45);
		}
	}
}
...
module gripper() {
	difference() {
		...
		translate([35.6,1.6,-0.5]) cube([2.7, 20.4, 20]);
		...
		translate([20,0.5,-1.1]) cube([3.6,4.8,10]);
		translate([21.8,2,3.1]) rotate(a=90, v=[1,0,0]) cylinder(r=1.5,h=5,$fn=45);
	...
	difference() {
		translate([-6,7,0])
			cube([10,10,10]);
		translate([-10,-0.1,2.88]) rotate([0,-12,0]) cube([38,24,11.5]);
	}
...
module gripperWithRack() {
translate([-20,5,0]) {
	rotate(a=-90, v=[0,1,0])
		translate([0.5,-8.8,-23.5])
			gripperRack();
...
module gripperRack() {
	difference() {
		...
		translate([-1,38,1.75])
			rotate(a=90, v=[0,1,0])
				cylinder(r=1.5, h=3, $fn=45);
	}
	translate([-3.7, 40.13 , 0])
		rotate(a=-30, v=[0,0,1])
			*cube([6.3, 3.5, 3.5]);

	for (i=[0:8]) {
		translate([1.3, 4 + 2.96*i, 0]) rackTooth();
	}
	translate([-1,5.7,1.7])
		rotate(a=90,v=[0,1,0]) {
			cylinder(r=1.45,h=3,$fn=45);
			translate([0,-0.2,0])
			cube([1.7,0.4,1]);
		}
